...
extern pthread_key_t key_handle;
extern snd_pcm_t *phandle;
extern snd_pcm_t *chandle;
int flag_Playing = 1;
...
printf("crashed here!");
    ...
    int channel = hwparams.channels;
    printf(" hwparams.channels = ", hwparams.channels);
	if (err < 0) {
    ...
    snd_pcm_hw_params_get_period_size(params, &chunk_size, 0);printf("get chunk_szie = %d\n",chunk_size);
    snd_pcm_hw_params_get_buffer_size(params, &buffer_size);printf("get buffer_size = %d\n",buffer_size);
...
static size_t pcm_read(snd_pcm_t *handle,snd_pcm_uframes_t chunk_size, u_char *data, size_t rcount)
{
	...
	while (count > 0) {
		...
		if (r == -EAGAIN || (r >= 0 && (size_t)r < count)) {
            ...
            xrun(handle,SND_PCM_STREAM_CAPTURE);
		} else if (r < 0) {
...
static ssize_t pcm_write(snd_pcm_t *handle,snd_pcm_uframes_t chunk_size, u_char *data)
{
	...
	while (count > 0) {
		...
		if (r == -EAGAIN || (r >= 0 && (size_t)r < count)) {
            ...
            xrun(handle,SND_PCM_STREAM_PLAYBACK);
        } else if (r < 0) {printf("crashed here\n");
...
void read_flag_playing(int *flag)
{
    pthread_rwlock_rdlock(&rwlock);
    *flag = flag_Playing;
    pthread_rwlock_unlock(&rwlock);
}
void write_flag_playing(int flag)
{
...
void playback(snd_pcm_t *handle,snd_pcm_uframes_t chunk_size)
{
	...
	do {
		if (pausing == 0) {
            ...
            printf("playback write start :%d\n",tEnd.tv_usec);
            pcm_write(handle,c, buf_tx);

			/* Looped playback */
            if (rd == 0)
            {
                write_flag_playing(0);
                break;//lseek(fd, 0, SEEK_SET);
	...
	free(buf_tmp);
	free(buf_tx);
    printf("flag_Playing = %d\n",flag_Playing);
    printf("playback end here!\n");
}

void capture(snd_pcm_t *handle, snd_pcm_uframes_t chunk_size)
{
    static int fd = -1;
    const char *name = "record.wav";//file_name;	/* current filename */
	...
	while (rest > 0) {
        ...
        gettimeofday(&tEnd, NULL);
        printf("capture read start :%d\n",tEnd.tv_usec);
        if (pcm_read(handle,chunk_size, buf_rx, f) != f)
			break;
        ...
        int flag = 1;
        read_flag_playing(&flag);
        if (flag == 0)
        {
            printf("capture need to end\n");
            break;//lseek(fd, 0, SEEK_SET);
...
static int run_duplex(snd_pcm_t *handle,snd_pcm_stream_t stream)
{
	...
	if (stream == SND_PCM_STREAM_PLAYBACK)
        playback(handle,chunk_size);
	else
        capture(handle,chunk_size);
...
int audio(snd_pcm_t *handle,int argc, char *argv[],int a)
{
	...
	if (duplex == 1) {
		...
		if (err < 0) {
            ...
            pthread_setspecific(key_handle,handle);
            printf("stream %handle @ ",stream,handle);
        }
